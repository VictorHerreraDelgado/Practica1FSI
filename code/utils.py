...
class BranchBound(Queue):
    ...
    # Primero obtenemos la key y luego nos encargamos de ordenarlo según eso
    def extend(self, items):
        self.A.extend(items)
        self.A.sort(key=lambda x: x.path_cost)
        #self.A.extend(sorted(items,key=lambda x: x.path_cost))
...
class Heuristic(Queue):
    ...
    def extend(self, items):
        self.A.extend(items)
        self.A.sort(key=lambda x: x.path_cost + self.problema.h(x))
